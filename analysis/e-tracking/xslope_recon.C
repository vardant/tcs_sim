...
void xslope_recon(string rootfile="RootFiles/test.rpt.root", int quarter=0,
		  string comment="test") {
  ...
  if (quarter==0 || quarter==3)
    h_dslope_vs_slope = new TH2F("h_dslope_vs_slope",
		     "#delta(slope) vs slope", 100, 0., 0.35, 100, -0.05, 0.05);
  else
    h_dslope_vs_slope = new TH2F("h_dslope_vs_slope",
		     "#delta(slope) vs slope", 100, -0.35, 0., 100, -0.05, 0.05);

  TH2F* h_dslope_vs_p = new TH2F("h_dslope_vs_p",
		     "#delta(slope) vs p", 100, 0., 10., 100, -0.05, 0.05);
  ...
  for (int ientry=0; ientry<nentries; ientry++) {
	...
	max_hitlist_size = hitlist.size();
      ...
      double xslope_det = atan(vdir.X()/vdir.Z());

      double xslope_recon = xslope_det;
