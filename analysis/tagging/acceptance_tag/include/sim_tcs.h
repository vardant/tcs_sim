...
// Sat Dec  8 09:10:48 2018 by ROOT version 6.02/08
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   sim_tcs(TTree *tree=0);
   sim_tcs(string file_name, TTree *tree=0);   ////
   virtual ~sim_tcs();
...
#ifdef sim_tcs_cxx
sim_tcs::sim_tcs(TTree *tree) : fChain(0) 
{
...
Int_t sim_tcs::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t sim_tcs::LoadTree(Long64_t entry)
{
...
void sim_tcs::Show(Long64_t entry)
{
...
Int_t sim_tcs::Cut(Long64_t entry)
{
...
sim_tcs::sim_tcs(string file_name, TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
...
Int_t    sim_tcs::GetEntriesFast() {
  return fChain->GetEntriesFast();
}

TTree* sim_tcs::GetTree() {
  return fChain;
}

#endif // #ifdef sim_tcs_cxx
