...
void hodox_hit_map(string hodo_tree, string dir, string phases, int maxfiles,
		  double e_thr, string comment) {

  TH1D* hTotHitPos = new TH1D("hh","top HodoX", NCHAN+1,-0.5,NCHAN+0.5);
  hTotHitPos->GetXaxis()->SetTitle("channel");
  hTotHitPos->GetYaxis()->SetTitle("Rate [Hz]");
  TH1D *hTotHitNeg = (TH1D*)hTotHitPos->Clone("hTotHitNeg");
  hTotHitNeg->SetTitle("bottom HodoX");
