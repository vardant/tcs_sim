...
void hodoy_hit_map(string hodo_tree, string dir, string phases, int maxfiles,
		  double e_thr, string comment) {

  TH1D* hTotHitPos = new TH1D("hh","top HodoY", NCHAN+1,-0.5,NCHAN+0.5);
  hTotHitPos->GetXaxis()->SetTitle("channel");
  hTotHitPos->GetYaxis()->SetTitle("Rate [Hz]");
  TH1D *hTotHitNeg = (TH1D*)hTotHitPos->Clone("hTotHitNeg");
  hTotHitNeg->SetTitle("bottom HodoY");

  Int_t nY = hTotHitPos->GetNbinsX();
  ...
  hTotHitNeg->Scale(CPSflux/double(nentries));
   ...
   TCanvas *ct = new TCanvas("tothit_map", comment.c_str(), 600, 720);
